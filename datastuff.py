...
def get_last_seen(userid: int, chatid: int):
    res = botstate.DBLink.execute("""
    SELECT last 
    FROM userseen 
    WHERE userid=? 
...
def get_newest_nick(userid: int, chatid: int) -> Optional[str]:
    res = botstate.DBLink.execute("""
    SELECT event_data 
    FROM user_events
    WHERE userid=?
...
def get_all_nicks(userid: int, chatid: int) -> list[str] | None:
    res = botstate.DBLink.execute("""
    SELECT event_data 
    FROM user_events
    WHERE userid=?
...
def get_times_joined(userid: int, chatid: int) -> list[str] | None:
    res = botstate.DBLink.execute("""
    SELECT event_data 
    FROM user_events
    WHERE userid=?
...
def get_join_date(userid: int, chatid: int) -> Optional[Tuple[float, bool]]:
    res = botstate.DBLink.execute("""
    SELECT time,fake
    FROM join_dates
    WHERE userid=? 
...
def get_quotes(userid: int, chatid: int) -> list[str] | None:
    if chatid == 0:
        res = botstate.DBLink.execute("""
            SELECT quote,chatid
            FROM qdb
...
def save_quote(userid: int, chatid: int, author: int, messageid: int, quote: string) -> int:
...
# --- { SCORE STUFF
def score_get_scope_strings(now: datetime = None):
...
def score_fetch(chatid: int, userid: int, scorename: string, now: datetime = None):
    today = now if now is not None else datetime.now()
    year = today.strftime("%Y")
    yearmonth = today.strftime("%Y-%m")
    yearmonthday = today.strftime("%Y-%m-%d")
...
def score_get_scoreboard(chatid: int, scorename: string, heading: string):
    outtext = heading
...
def score_add(chatid: int, userid: int, scorename: string, delta: int):
    today = datetime.now()
    year = today.strftime("%Y")
    yearmonth = today.strftime("%Y-%m")
    yearmonthday = today.strftime("%Y-%m-%d")
...
def score_fetch_scope(chatid: int, userid: int, scorename: string, scope: string):
    res = botstate.DBLink.execute("""
    SELECT amount 
    FROM scores 
    WHERE userid = ? 
...
def score_fetch_top_scope(chatid: int, scorename: string, count: int = 1, scope: string = "all"):
    res = botstate.DBLink.execute("""
...
def score_add_scope(chatid: int, userid: int, scorename: string, scope: string, delta: int):
...
def fmt_timespan(timespan: float) -> str:
...
def listactive(chatid: int, count: int = 10) -> str:
...
def show_all_quotes(userid: int, chatid: int) -> str:
...
def whois(userid: int, chatid: int) -> str:
    print("---")
...
# log an event
def log_user_event(userid: int, chatid: int, event_type: string, data: string):
    last = time.time()
...
def changerep(userid: int, chatid: int, delta: int) -> int:
...
# changes a given user's reputation score
def upcountmessage(userid: int, chatid: int) -> int:
...
# joining users
def handle_join(upd: Update) -> string:
...
# leaving users
def handle_leave(upd: Update) -> string:
    userid = upd.chat_member.new_chat_member.user.id
    chatid = upd.effective_chat.id
    log_user_event(userid=userid, chatid=chatid, event_type="left", data="")


def handle_new_user(userid: int, chatid: int, unknown_date: bool = False):
    last = time.time()
    botstate.DBLink.execute("""
...
def handle_bot_cleaner(userid: int, chatid: int, msgid: int):
...
def add_message_to_pool(chatid: int, name:str,pool_id: string, weight: float = 1.0):
...
async def retrieve_message_from_pool(chatid:int,target_chatid:int,pool_id:str,reply_to_id:int =0,try_global: bool = True):
    res = botstate.DBLink.execute(("""
    SELECT message_name,weight FROM message_pools
    WHERE pool_id = ?
    AND chatid = ?
...
def write_message_component(chatid: int, name: string, component_type: string, data: string, userid: int):
    botstate.DBLink.execute("""
...
def get_message_components(chatid: int, name: string):
    res = botstate.DBLink.execute("""
...
async def save_message(name: string, msg: Message, userid: int, chatid: int):
...
async def retrieve_message(name: string, chatid: int,  do_global: bool = False, target_chatid: int = 0,reply_to_id:int = 0 ):
    """
...
""".format(qname=quizname)
                for result in results:
                    ...
                    timing = result[1]
                    if medalcouner < 3:
                        medal = medals[medalcouner]
                    else:
                        score_add(chatid=chatid, userid=uid, scorename="quiz_medals_other", delta=1)
                    usernick = get_newest_nick(userid=uid, chatid=chatid)
                    score_add(chatid=chatid,userid=uid,scorename="quiz_medals_"+str(medalcouner),delta=1)
                    score_add(chatid=chatid,userid=uid,scorename="quiz_participations",delta=1)
                    if not usernick:
                        usernick = "MissingNo.&%!▮"
                    if len(usernick) > 15:
                        usernick="{:.12}...".format(usernick)
                    usernick = MD(usernick,1)
                    resultrow = medal + "*" + usernick + "*: _" + str(score) + "_ правильных за _" + str(round(timing, 1)) + "_ секунд.\n"
                    msgtpl += resultrow
                    medalcouner += 1
